...
public class VotingDAO implements IVotingDAO {
    ...
    public Map<Nomination, List<Candidate>> getNominationsWithCandidates() {
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement stmt = connection.prepareStatement(
                    "SELECT c.id as candidate_ID," +
                            ...
                            "ORDER BY RANDOM();")) {
                connection.setAutoCommit(true);
                Map<Nomination, List<Candidate>> result = new HashMap<>();
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        Nomination nomination = new Nomination(rs.getLong("nomination_id"),
                                rs.getString("nomination_name"),
                                rs.getInt("nomination_order"));
                        Candidate candidate = new Candidate(rs.getString("about_candidate"),
                                rs.getString("candidate_name"),
                                rs.getString("displayed_name"),
                                rs.getLong("nomination_id"));
                        candidate.setId(rs.getLong("candidate_ID"));
                        if (result.get(nomination) != null) {
                            result.get(nomination).add(candidate);
                        } else {
                            List<Candidate> cands = new ArrayList<>();
                            cands.add(candidate);
                            result.put(nomination, cands);
                        }
                    }
                }
                return result;
    ...
    public Map<Nomination, List<Candidate>> getNominationsWithCandidatesAndVotes() {
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement stmt = connection.prepareStatement(
                    "SELECT c.id as candidate_ID,\n" +
                            " c.username as candidate_name,\n" +
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        candidate.setVoteCountFinished(rs.getInt("vote_count_finished"));
                        if (result.get(nomination) != null) {
                            result.get(nomination).add(candidate);
                        } else {
                            List<Candidate> cands = new ArrayList<>();
